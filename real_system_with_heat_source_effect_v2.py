...
model1 = mm.MKModel(model_id= 1, model_size=(10,40,100), D = 5e-5, initial_condition = 25)
model2 = mm.MKModel(model_id= 2, model_size=(5, 30, 30), D = 3.4e-7, initial_condition = 25) # glass
model3 = mm.MKModel(model_id= 3, model_size=(10, 20, 20), D = 3.1e-5, initial_condition = 25) # GaAs P-N junction semiconductor

model4 = mm.MKModel(model_id= 4, model_size=(5, 30, 30), D = 3.4e-7, initial_condition = 25) # glass
model5 = mm.MKModel(model_id= 5, model_size=(10, 20, 20), D = 3.1e-5, initial_condition = 25) # GaAs P-N junction semiconductor


main_system = mm.MKMultiMaterialSystem(base_model = model1, delta_pos=voxel_L, delta_time= 1e-8, time_step_scaling = 1,\
                                       padding=(3, 0.2, 0.2))
main_system.addModel(model2, onTopOfObject=model1, placement_position=(0,0,-30))
main_system.addModel(model3, onTopOfObject=model2)

main_system.addModel(model4, onTopOfObject=model1, placement_position=(0,0,30))
main_system.addModel(model5, onTopOfObject=model4)
...
main_system.imposeHeatSource(heat_source_id=305, mat_id=5, Q=Q,c=C_joul_per_gram,rho=rho_GaAs,\
                              positionOffset=(5,0,0))
...
mm.plot2DImage(2, main_system.main_model[:,25,:], xAxisLabel= 'X (10x um)', yAxisLabel= 'Z (10x um)', showContour=False )
mm.plot2DImage(21, np.nan_to_num(u_init[:,25,:]), xAxisLabel= 'X (10x um)', yAxisLabel= 'Z (10x um)' , showContour= False)
...
mm.plot2DImage(27, main_system.D[:,25,:]*1e4, xAxisLabel= 'X (10x um)', yAxisLabel= 'Z (10x um)', showContour= True)
...
data, mid_temp = main_system.runSimulation_v2(iterationTime=20000, initU=u_init, figureID=401, deltaTime= 1e-7,\
                                    ySection= 25, applyDiri=True, applyNeumann=True, \
                                    shutDownExternalHeatingAtTime=1*1e443, shutDownDuration=1e100) # , getZSectionData =
