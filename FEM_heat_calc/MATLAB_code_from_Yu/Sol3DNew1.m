...
for i=1:My
   ...
   % Newman BC
   if (floor(i/10)==2)|(i==30)&(floor(j/8)==2)|(j==24)
       u(i,19,j)=u(i,20,j);
       u(i,31,j)=u(i,30,j);
   end
   
   if (floor(h/10)==2|h==30)&(floor(j/8)==2)|(j==24)
       u(19,h,j)=u(20,h,j);
       u(31,h,j)=u(30,h,j);
   end
   
   if (floor(i/18)==1)&(i~=33)&(i~=34)&(i~=35)&(floor(j/6)==1)|(j==12)|(j==13)|(j==14)|(j==15)
       u(i,17,j)=u(i,18,j);
       u(i,33,j)=u(i,32,j);
   end
   
   if (floor(h/18)==1)&(h~=33)&(h~=34)&(h~=35)&(floor(j/6)==1)|(j==12)|(j==13)|(j==14)|(j==15)
       u(17,h,j)=u(18,h,j);
       u(33,h,j)=u(32,h,j);
   end
...
%     s(:,:,Mx+1)=s(:,:,Mx);
    ...
    % Iteration
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  junction PN
    if (floor(i/4)==5)&(floor(h/10)==2|h==30)&(floor(j/8)==2)|(j==24)                 
    ...
    if (floor(i/3)==8)&(floor(h/4)==5)&(floor(j/8)==2)|(j==24)
             u(i,h,j)=u_old(i,h,j)+ry2*(u_old(i+1,h,j)+u_old(i-1,h,j)-2*u_old(i,h,j))+rx2*(u_old(i,h,j+1)+u_old(i,h,j-1)-2*u_old(i,h,j))+rz2*(u_old(i,h+1,j)+u_old(i,h-1,j)-2*u_old(i,h,j));
             s(i,h,j)=u(i,h,j);
    end
    
    if (floor(i/3)==8)&(floor(h/3)==8)&(floor(j/8)==2)|(j==24)
    ...
    if (floor(i/3)==8)&(floor(h/3)==9|h==30)&(floor(j/8)==2)|(j==24)
             u(i,h,j)=u_old(i,h,j)+ry2*(u_old(i+1,h,j)+u_old(i-1,h,j)-2*u_old(i,h,j))+rx2*(u_old(i,h,j+1)+u_old(i,h,j-1)-2*u_old(i,h,j))+rz2*(u_old(i,h+1,j)+u_old(i,h-1,j)-2*u_old(i,h,j));
             s(i,h,j)=u(i,h,j);
    end
    
    if (floor(i/3)==9|i==30)&(floor(h/3)==9|h==30)&(floor(j/8)==2|j==24)
            u(i,h,j)=u_old(i,h,j)+ry2*(u_old(i+1,h,j)+u_old(i-1,h,j)-2*u_old(i,h,j))+rx2*(u_old(i,h,j+1)+u_old(i,h,j-1)-2*u_old(i,h,j))+rz2*(u_old(i,h+1,j)+u_old(i,h-1,j)-2*u_old(i,h,j));
            s(i,h,j)=u(i,h,j);
    end
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    la Colle
...
%     end
    ...
    if (floor(i/18)==1)&(i~=33)&(i~=34)&(i~=35)&(floor(h/18)==1)&(h~=33)&(h~=34)&(h~=35)&(floor(j/6)==1)|(j==12)|(j==13)|(j==14)|(j==15)
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% lead frame 
    if (floor(i/50)==0)&(i~=1)&(floor(h/50)==0)&(h~=1)
