...
Mz = 104;
N = 4000;                     % ot<=2.5us
ro1 = 6.15e-12;               % g/um^3 masse volumique
% ro2 = ...;
% ro3 = ...;
% ro4 = ...;
cc1 = 0.49;                   % J/g/K  chaleur specifique
% cc2 = ...;
% cc3 = ...;
% cc4 = ...;
I1 = 0.01;                     % A
...
% I3 = ...;
% I4 = ...;
U1 = 2.5;                     % V tension de seuil
% U2 = ...;
% U3 = ...;
% U4 = ...;
R1 = 20;                      % ohm resistance equivalente
% R2 = ...;
% R3 = ...;
% R4 = ...;
D11 = 43;                    % Junction PN GaN
% D21 = ...;
% D31 = ...;
% D41 = ...;
D2 = 161.8;                  % Glu
...
oz = (L(6)-L(5))/Mz;
z = L(5)+0:Mz*oz;
P1 = I1^2*R1/36/ox^3/ro1/cc1*1e-6;   %JPN+I^2R:4.5 5.5 6.5 7.5
% P2 = ...;
% P3 = ...;
% P4 = ...;
Pv1 = (I1*U1)/9/ox^3/ro1/cc1*1e-6;  %JPN+UI:4.6 5.6 6.6 7.6
% Pv2 = ...;
% Pv3 = ...;
% Pv4 = ...;
...
%   c=100/alpha*(square(2.*pi./12.*t,alpha)+1)/2;
    ...
    u(JPNc1)=u_old(JPNc1)+ry11*(u_old(w11)+u_old(w12)-2*u_old(JPNc1))+rz11*(u_old(w13)+u_old(w14)-2*u_old(JPNc1))+rx11*(u_old(w15)+u_old(w16)-2*u_old(JPNc1));%+P1*i^2*ot;
    
    u(JPNc2)=u_old(JPNc2)+ry21*(u_old(w21)+u_old(w22)-2*u_old(JPNc2))+rz21*(u_old(w23)+u_old(w24)-2*u_old(JPNc2))+rx21*(u_old(w25)+u_old(w26)-2*u_old(JPNc2))+P1*i^2*ot;%+P2*ot;
    ...
    % Pv=U*I
    u(ZEC1)=u_old(ZEC1)+ry11*(u_old(m11)+u_old(m12)-2*u_old(ZEC1))+rz11*(u_old(m13)+u_old(m14)-2*u_old(ZEC1))+rx11*(u_old(m15)+u_old(m16)-2*u_old(ZEC1));%+Pv1*i*ot;
    
    u(ZEC2)=u_old(ZEC2)+ry21*(u_old(m21)+u_old(m22)-2*u_old(ZEC2))+rz21*(u_old(m23)+u_old(m24)-2*u_old(ZEC2))+rx21*(u_old(m25)+u_old(m26)-2*u_old(ZEC2))+Pv1*i*ot;%+Pv2*ot;
