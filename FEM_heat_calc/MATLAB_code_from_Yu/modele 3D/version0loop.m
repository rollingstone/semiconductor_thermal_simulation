...
for i=20:30
    h=19:2;
    j=16:Mx-1;
    f(i,h,j)=2.1;%flag{i,19,j}={'NJpn1'};%,u(i,20,j)};           % 2-------Neumman pour la Jonction PN
end

for i=20:30
    h=31:Mz-1;
    j=16:Mx-1;
    f(i,h,j)=2.2;%flag{i,31,j}={'NJpn2'};%,u(i,30,j)};
end
...
%     end
       ...
       G(fram,1)=u(25,25,5);                  % temperature du pixel fix�� 
       G(fram,2)=u(25,25,10);
       G(fram,3)=u(25,25,20);
       G(fram,4)=T1;                          % temp��rature moyenne du LF
       G(fram,5)=T2;                                              % de la colle 
       G(fram,6)=T3;                                              % de la JPN
       G(fram,7)=0.1*sqrt((100/alpha)*(square(2.*pi./12.*t,alpha)+1)/2);    % courant 
