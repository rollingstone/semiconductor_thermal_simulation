...
oz = (L(6)-L(5))/Mz;
z = L(5)+0:Mz*oz;
P1 = I1^2*R1/18/ox^3/ro1/cc1*1e-6;   %I^2R:4.5 5.5 6.5 7.5
% P2 = ...;
% P3 = ...;
% P4 = ...;
Psi = I1^2*Rsi/50/ox^3/rosi/ccsi*1e-6;
Psap = I1^2*Rsap/50/ox^3/rosap/ccsap*1e-6;
Pv1 = (I1*U1)/9/ox^3/ro1/cc1*1e-6;  %UI:4.6 5.6 6.6 7.6
% Pv2 = ...;
% Pv3 = ...;
% Pv4 = ...;
...
%     end
   c=0.3*(square(2.*pi./10000.*t,50)+1)/2+0.7;
    ...
    % la substrate Si
    u(Si)=u_old(Si)+rysi*(u_old(s1)+u_old(s2)-2*u_old(Si))+rzsi*(u_old(s3)+u_old(s4)-2*u_old(Si))+rxsi*(u_old(s5)+u_old(s6)-2*u_old(Si))+Psi*ot.*c^2;
