...
NJPNC=find(f(:,:,16)==0);                                   % 4---------Neumman pour JPN-colle
f(2500*15+NJPNC)=4;
NcLf=find(f(:,:,6)==0);                                     % 5---------Neumman pour colle-Lf
f(2500*5+NcLf)=5;
...
for i=20:30
    j=16:Mx-1;                                         % 10------------Junction PN + courent
       f(i,25,j)=10;%flag{i,h,j}={'JPN+c'};%,u(i,h,j)+ry2*(u(i+1,h,j)+u(i-1,h,j)-2*u(i,h,j))+rx2*(u(i,h,j+1)+u(i,h,j-1)-2*u(i,h,j))+rz2*(u(i,h+1,j)+u(i,h-1,j)-2*u(i,h,j))+P.*ot.*c};
end
...
for h=20:30
 ...
 %Dirichlet
 Diri=find(f==1);
 [yd,zd,xd]=ind2sub([My,Mz,Mx],Diri);
 d1=sub2ind([My,Mz,Mx],yd,zd,xd+1);
...
%  N12=find(f==12);
%  [y12,z12,x12]=ind2sub([My,Mz,Mx],N12);
%  o1=sub2ind([My,Mz,Mx],y12,z12,x12+1);
...
%   c=100/alpha*(square(2.*pi./12.*t,alpha)+1)/2;
    ...
    u(ZEC)=u_old(ZEC)+ry2*(u_old(m1)+u_old(m2)-2*u_old(ZEC))+rz2*(u_old(m3)+u_old(m4)-2*u_old(ZEC))+rx2*(u_old(m5)+u_old(m6)-2*u_old(ZEC))+Pv.*ot+P*ot;
