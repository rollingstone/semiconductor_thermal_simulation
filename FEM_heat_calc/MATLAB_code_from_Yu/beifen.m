...
for k=1:N
    u_1 = u; t = k*ot;
    for i=1:My+1
        u(i,1) = 50;%feval(u_xyt,x(1),y(i),t);
        u(i,Mx+1) = u(i,Mx);%feval(u_xyt,x(Mx+1),y(i),t);
    end
    for j=1:Mx+1
        u(1,j) = u(2,j);%(j<=(Mx+1)/2).*feval(u_xyt,x(j),y(1),t)+(j>(Mx+1)/2).*u(2,j);
        u(My+1,j) = u(My,j);%feval(u_xyt,x(j),y(My+1),t);
    end
    
    if mod(k,2)==0
        for i=2:My
            jj=2:Mx;
            bx=[ry*u(i,1),zeros(1,Mx-3),ry*u(i,Mx+1)]+rx*(u_1(i-1,jj)+u_1(i+1,jj))+rx2*u_1(i,jj);
            u(i,jj)=linsolve(A, bx');
        end 
    else
        for j=2:Mx
                ii=2:My;
                by=[rx*u(1,j);zeros(My-3,1);rx*u(My+1,j)]+ry*(u_1(ii,j-1)+u_1(ii,j+1))+ry2*u_1(ii,j);
